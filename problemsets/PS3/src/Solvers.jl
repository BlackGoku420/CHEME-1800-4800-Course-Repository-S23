...
"""
_jacobi_iteration_solver(model::MyChemicalDecayModel, xₒ::Array{Float64,1},
...
"""
function _jacobi_iteration_solver(model::MyChemicalDecayModel, xₒ::Array{Float64,1},
    tolerance::Float64, maxiter::Int64)::Array{Float64,1}

    # initialize
    (number_of_cols) = size(model.A,1);
...
"""
_gauss_seidel_iteration_solver(model::MyChemicalDecayModel, xₒ::Array{Float64,1},
    tolerance::Float64, maxiter::Int64) -> Array{Float64,1}
...
"""
function _gauss_seidel_iteration_solver(model::MyChemicalDecayModel, xₒ::Array{Float64,1},
    tolerance::Float64, maxiter::Int64)::Array{Float64,1}

    # Step 4: Check out the documentation for IterativeSolvers.jl
    # see: https://iterativesolvers.julialinearalgebra.org/stable/
    return gauss_seidel(model.A,model.b; maxiter = maxiter)
end

"""
solve(type::T, model::MyChemicalDecayModel, xₒ::Array{Float64,1};
...
function solve(type::T, model::MyChemicalDecayModel, xₒ::Array{Float64,1};
    ...
    if (isa(type, JacobiIterationSolver) == true)
        return _jacobi_iteration_solver(model, xₒ, tolerance, maxiter);
    elseif (isa(type, GaussSeidelIterationSolver) == true)
        return _gauss_seidel_iteration_solver(model, xₒ, tolerance, maxiter);
    else
